...
export const ScheduledRecordingsFullPage: React.FC = () => {
                    ...
                    <tr key={`${row.scheduleKey}-${index}`} className="hover:bg-emerald-50/40 transition-colors duration-200">
                      ...
                      <td className="px-3 py-1 whitespace-nowrap text-xs">
                        <span className={`inline-block px-2 py-0.5 rounded-full text-[10px] font-medium ${
                          ...
                          row.status === 'ready_to_transmit' ? 'bg-green-100 text-green-700' :
                          row.status === 'transmitting' ? 'bg-orange-100 text-orange-700' :
          ...
          <div className="p-4 bg-gray-50 border-t border-gray-200 flex justify-end">
            <button
              onClick={() => {
                ...
                link.setAttribute('download', `scheduled_recordings_${serverName.replace(/\s+/g, '_')}_${clientName.replace(/\s+/g, '_')}_${new Date().toISOString().split('T')[0]}.csv`);
