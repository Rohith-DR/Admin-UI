          ...
          >
            ...
            <div className="bg-white rounded border border-gray-200 flex-1 flex flex-col overflow-hidden min-h-0">
              ...
              <div className="flex-1 bg-gray-900 flex items-center justify-center p-2 overflow-auto min-h-0">
                {loadingDetails || (selectedAudio && !selectedDetails) ? (
                  <div className="text-center">
                    <Loader2 className="w-6 h-6 animate-spin text-cyan-400 mx-auto" />
                    <p className="text-gray-400 text-[10px] mt-2">Loading spectrogram...</p>
                  </div>
                  ...
                  <div className="text-gray-500 text-center">
                    ...
                    <p className="text-[10px] mt-1">Click on a file to view spectrogram</p>
                  </div>
                ) : (
                  <div className="text-center">
                    <Loader2 className="w-6 h-6 animate-spin text-cyan-400 mx-auto" />
                  ...
                  <div>
                    ...
                    {openSections.metadata && (
                      <div className="px-2 pb-2 space-y-0.5 text-[10px]">
                        ...
                        {metadata.timestamp && <div className="flex justify-between py-0.5"><span className="text-gray-400">Time</span><span className="text-gray-700 text-right max-w-[70%] break-words">{metadata.timestamp}</span></div>}
                        ...
                        {metadata.firmware && <div className="flex justify-between py-0.5"><span className="text-gray-400">Firmware</span><span className="text-gray-700">{metadata.firmware}</span></div>}
                        ...
                        {metadata.species && <div className="flex justify-between py-0.5"><span className="text-gray-400">Species ID</span><span className="text-gray-700">{metadata.species}</span></div>}
                        {metadata.note && <div className="flex justify-between py-0.5"><span className="text-gray-400">Note</span><span className="text-gray-700 text-right max-w-[60%] break-words">{metadata.note}</span></div>}
                        
                        {/* Additional raw GUANO fields not in primary list */}
                        {metadata.raw_metadata && Object.entries(metadata.raw_metadata).map(([key, value]) => {
                          // Skip fields we've already displayed and internal fields
                          const displayedFields = ['Timestamp', 'GUANO|Timestamp', 'Loc Position', 'GUANO|Loc Position', 
                            'Length', 'GUANO|Length', 'TE', 'GUANO|TE', 'Temperature Ext', 'GUANO|Temperature Ext',
                            'Humidity', 'GUANO|Humidity', 'Make', 'GUANO|Make', 'Model', 'GUANO|Model',
                            'Firmware Version', 'GUANO|Firmware Version', 'Filter HP', 'GUANO|Filter HP',
                            'Filter LP', 'GUANO|Filter LP', 'Species Manual ID', 'GUANO|Species Manual ID',
                            'Note', 'GUANO|Note'];
                          
                          if (displayedFields.includes(key) || !value) return null;
                          
                          // Clean up key for display (remove GUANO| prefix if present)
                          const displayKey = key.replace(/^GUANO\|/, '');
                          
                          return (
                            <div key={key} className="flex justify-between py-0.5">
                              <span className="text-gray-400">{displayKey}</span>
                              <span className="text-gray-700 text-right max-w-[60%] break-words">{String(value)}</span>
                            </div>
                          );
                        })}
